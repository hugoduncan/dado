...
(defn- exec-result->message [tool-call exec-result]
  (message/tool-result-content
   {:tool-use-id (have string? (:id tool-call))
    :content     (:content exec-result)
    :is-error    (:is-error exec-result)}))
   ...
   (let [tool (have (tools (name (have (:tool tool-call)))))
     ...
     (when result
       ...
       {:context-mod         context-mod
        :tool-result-content (exec-result->message tool-call result)}))))
...
(defn- add-tool-results
  [msg-thread tool-results]
  (let [tool-result-contents (t/spy!
                              {:id ::add-tool-results}
                              (into [] (keep :tool-result-content) tool-results))
        context-mods         (into [] (keep :context-mod) tool-results)
        msg-thread           (if (seq tool-result-contents)
                               (message/add-message
                                msg-thread
                                (t/trace!
...
(defn complete-with-tools!
  ...
  (loop [msg-thread msg-thread]
    ...
    (let [_ (t/event! :message-loop/sending-request)
          ...
          tool-results (execute-tool-calls! msg-thread tool-calls)
      ...
      (if (seq tool-results)
        (recur (add-tool-results msg-thread tool-results))
        msg-thread))))
